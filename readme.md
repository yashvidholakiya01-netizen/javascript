...
<div id="box">  --> identify it
  <h1>Hello</h1> --> brower show
  <p>Welcome</p>
</div>
...
// 6. What does getElementsByClassName return? Is it an array?
 No, it is NOT a real array.
 It is an array-like object (HTMLCollection).
That means:
It has index numbers (like 0, 1, 2)
It has a length property

// 7. Use querySelectorAll to select all buttons with class "buy-now".
To select all buttons with class "buy-now", use:
document.querySelectorAll("button.buy-now");

// 8. T1: Select the heading of a page by ID and change its text to "Welcome to JS DOM"
document.getElementById("mainHeading").textContent = "Welcome to JS DOM";
...
// list.forEach((val)=>{
    ...
    // }
ans:
querySelectorAll("li") selects all <li> elements.
It returns a NodeList.
We loop through it using forEach() or a for loop.
.textContent gets the text inside each <li>.
